...
class Service {
  ...
  _selector(params) {
    const allowedKeys = ['selector', 'limit', 'skip', 'sort', 'fields', 'use_index', 'r', 'bookmark', 'update', 'stable', 'stale', 'execution_stats'];

    return _bluebird2.default.promisify(this.connection.request)({
      method: 'POST',
      doc: '_find',
      db: this.database,
      body: (0, _util.filterByKeys)(allowedKeys, params)
    });
  }
