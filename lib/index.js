...
const removeInvalidParams = params => {
  const allowedParams = ['selector', 'limit', 'skip', 'sort', 'fields', 'use_index', 'r', 'bookmark', 'update', 'stable', 'stale', 'execution_stats'];
  ...
  */
  find(params = {}) {
    ...
    const viewOrList = params => {
      const query = _extends({ include_docs: true }, params);
      return (params.view ? view(query) : list(query)).then(res => ({
